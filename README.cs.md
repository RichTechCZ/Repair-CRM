...
CREATE USER '<db_user>'@'localhost' IDENTIFIED BY '<strong-db-password>';
GRANT ALL PRIVILEGES ON repair_crm.* TO '<db_user>'@'localhost';
FLUSH PRIVILEGES;
```
...
Vytvořte první administrátorský účet z příkazové řádky. Hodnoty nastavte pouze v shellu nebo prostředí serveru, ne v Gitu:

```bash
CRM_ADMIN_USERNAME="<admin-user>" CRM_ADMIN_PASSWORD="<strong-admin-password>" php tools/create_admin.php
```

Poté otevřete `https://your-domain.com/login.php` a přihlaste se vytvořeným účtem.
